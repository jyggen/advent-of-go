...
func parseHandWithJack(input string) hand {
...
func parseHandWithJoker(input string) hand {
	delimiterAt := strings.IndexRune(input, ' ')
	cardIdentifiers := []rune(input[:delimiterAt])
	bid, _ := strconv.Atoi(input[delimiterAt+1:])
	h := hand{
...
func parseHands(input string, parseHandFunc func(string) hand) []hand {
	rows := utils.ToStringSlice(input, "\n")
	hands := make([]hand, 0, len(rows))

	for _, r := range rows {
		hands = append(hands, parseHandFunc(r))
	}
...
func SolvePart1(input string) (string, error) {
	hands := parseHands(input, parseHandWithJack)
	total := 0
...
func SolvePart2(input string) (string, error) {
	hands := parseHands(input, parseHandWithJoker)
	total := 0
